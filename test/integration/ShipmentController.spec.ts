...
describe('ShipmentController (integración)', () => {
  ...
  function buildApp(createUseCase: Partial<CreateShipmentUseCase>, getByIdUseCase: Partial<GetShipmentByIdUseCase>, withUserId = true) {
    const ctrl = new ShipmentController(createUseCase as any, getByIdUseCase as any);
    const server = express();
    server.use(express.json());

    server.use((req, _res, next) => {
      req.user = { id: withUserId ? 'test-user' : '' };
      next();
    });
  ...
  it('POST /api/shipments → 201 y retorna el envío creado en caso exitoso', async () => {
    ...
    app = buildApp(createFakeUC, getFakeUC);
  ...
  it('POST /api/shipments → 400 y mensaje de error sin userId', async () => {
    createFakeUC = { execute: jest.fn() };
    app = buildApp(createFakeUC, getFakeUC, false);
  ...
  it('GET /api/shipments/:id → 404 y retorna un error si no se encuentra el envío', async () => {
    getFakeUC = {
